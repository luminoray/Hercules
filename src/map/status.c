	...
	for(i=0;i<EQI_MAX;i++) {
		...
		if(sd->inventory_data[index]->type == IT_WEAPON) {
			...
			if (sd->status.inventory[index].card[0]==CARD0_FORGE) {
				...
				if (wd->star == 5) {wd->star = 15;}
				else if (wd->star == 10) {wd->star = 25;}
				else if(wd->star >= 15) {wd->star = 30;} // 3 Star Crumbs now give +30 dmg
...
short status_calc_aspd(struct block_list *bl, struct status_change *sc, short flag) {
	...
	if (flag&1) {
		...
		if (!sc->data[SC_QUAGMIRE]) {
			if(sc->data[SC_TWOHANDQUICKEN] && bonus < 14)
				bonus = 14;
			if(sc->data[SC_ONEHANDQUICKEN] && bonus < 14)
				bonus = 14;
			if(sc->data[SC_MER_QUICKEN] && bonus < 14) // needs more info
				bonus = 14;
			if(sc->data[SC_ADRENALINE2] && bonus < 12)
				bonus = 12;
			if(sc->data[SC_ADRENALINE] && bonus < 14)
				bonus = 14;
			if(sc->data[SC_SPEARQUICKEN] && bonus < 14)
				bonus = 14;
			if(sc->data[SC_HLIF_FLEET] && bonus < 10)
				bonus = 10;
		}
